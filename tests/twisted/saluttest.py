...
import salutservicetest
from salutservicetest import call_async, EventPattern
...
def make_connection(bus, event_func, params=None):
    ...
    return salutservicetest.make_connection(bus, event_func, 'salut',
...
def exec_test_deferred (fun, params, protocol=None, timeout=None,
    ...
    if sys.stdout.isatty() or 'CHECK_FORCE_COLOR' in os.environ:
        colourer = salutservicetest.install_colourer()

    bus = dbus.SessionBus()

    queue = salutservicetest.IteratingEventQueue(timeout)
    queue.verbose = (
